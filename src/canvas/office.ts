...
export const ZONES: OfficeZone[] = [
  { name: "desks", x: 60, y: 60, w: 400, h: 280, color: "#1a1a3e", label: "Work Area", icon: "💻" },
  { name: "bulletin", x: 520, y: 60, w: 300, h: 200, color: "#2a1a1a", label: "Forum Board", icon: "📋" },
  { name: "bookshelf", x: 520, y: 320, w: 300, h: 200, color: "#1a2a1a", label: "Knowledge Base", icon: "📚" },
  { name: "taskboard", x: 880, y: 60, w: 260, h: 280, color: "#2a2a1a", label: "Task Board", icon: "📌" },
  { name: "lounge", x: 60, y: 400, w: 400, h: 280, color: "#1a1a2a", label: "Lounge", icon: "☕" },
  { name: "shop", x: 880, y: 400, w: 260, h: 280, color: "#2a1a2a", label: "Shop", icon: "🛒" },
];
...
export function drawOffice(ctx: CanvasRenderingContext2D) {
  ...
  for (const zone of ZONES) {
    ...
    // Zone furniture/decorations
    drawZoneDetails(ctx, zone);
    ...
    ctx.textAlign = "left";
    ctx.fillText(`${zone.icon} ${zone.label}`, zone.x + 8, zone.y + 16);
    ctx.restore();
  }
  ...
  ctx.fillText("ENTRANCE", OFFICE_WIDTH / 2, OFFICE_HEIGHT - 8);
}

function drawZoneDetails(ctx: CanvasRenderingContext2D, zone: OfficeZone) {
  ctx.save();
...
export function updateAgentPosition(agent: AgentPosition, speed: number = 0.03): AgentPosition {
  ...
  if (dist < 2) {
    // Arrived at target, pick new random target within zone
    if (Math.random() < 0.01) {
